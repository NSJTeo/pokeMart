...
axios.get("https://pokeapi.co/api/v2/item-category/27/").then((response) => {
  ...
  itemsArray.forEach((item) => {
    axios.get(item.url).then((response) => {
      ...
      itemListElement.classList.add("shopping-menu-item__container");

      shoppingMenu.appendChild(itemListElement);

      const itemButton = document.createElement("button");
      // itemButton.classList.add("shopping-menu__add-button");
      itemButton.classList.add("shopping-menu-item__add-button");
      ...
      itemButton.addEventListener("click", () => {
        ...
        if (shoppingCartItems.hasOwnProperty(pokeItem.englishName)) {
          ...
          shoppingCartItems[pokeItem.englishName] = {
            count: 1,
            img: pokeItem.img,
            cost: pokeItem.cost,
            name: pokeItem.englishName,
          };
        }

        for (const item in shoppingCartItems) {
          totalCost +=
            shoppingCartItems[item].cost * shoppingCartItems[item].count;

          const shoppingCartItemContainer = document.createElement("section");
          shoppingCartItemContainer.classList.add(
            "shopping-cart__item-container"
          );
          ...
          shoppingCartDeleteItemButton.classList.add(
            "shopping-cart__delete-item-button"
          );
      ...
      itemImageContainer.classList.add("shopping-menu-item__image-container");
      ...
      itemImagePointer.classList.add("shopping-menu-item__pointer");
      ...
      itemImage.classList.add("shopping-menu-item__image");
      ...
      itemJapaneseName.classList.add("shopping-menu-item__name");
      itemJapaneseName.classList.add("shopping-menu-item__name--japanese");
      ...
      itemEnglishName.classList.add("shopping-menu-item__name");

      itemListElement.appendChild(itemEnglishName);

      const itemDescription = document.createElement("p");
      // itemDescription.classList.add("shopping-menu__item-description");
      itemDescription.classList.add("shopping-menu-item__description");
      ...
      itemPrice.classList.add("shopping-menu-item__price");
